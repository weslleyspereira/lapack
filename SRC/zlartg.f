...
*     LOGICAL            FIRST
     ...
     $                   SAFMN2, SAFMX2, SCALE, noise1, noise2
      COMPLEX*16         FF, FS, GS
*     ..
*     .. External Functions ..
...
*
   ...
   20    CONTINUE
     ...
     $      GO TO 20
      ...
      F2 = ABSSQ( FS ) * (1 + 1e-6*noise1) * (1 + 1e-6*noise2)
      call gaussian_dist( noise1, noise2 )
      G2 = ABSSQ( GS ) * (1 + 1e-6*noise1) * (1 + 1e-6*noise2)
...
*        Do the F2S(real)*FS(complex) multiply with two real multiplies
         R = DCMPLX( F2S*DBLE( FS ), F2S*DIMAG( FS ) )*(1 + 1e-6*noise2)
         call gaussian_dist( noise1, noise2 )
         CS = (ONE / F2S) * (1 + 1e-6*noise1)
         D = (F2 + G2) * (1 + 1e-6*noise2)
*        Do complex/real division explicitly with two real divisions
         call gaussian_dist( noise1, noise2 )
         SN = DCMPLX( DBLE( R ) / D, DIMAG( R ) / D )*(1 + 1e-6*noise1)
         call gaussian_dist( noise1, noise2 )
         SN = SN*DCONJG( GS ) * (1 + 1e-6*noise1) * (1 + 1e-6*noise2)
         IF( COUNT.NE.0 ) THEN
            IF( COUNT.GT.0 ) THEN
               DO 30 I = 1, COUNT
                  R = R*SAFMX2
...
*
      ...
      subroutine gaussian_dist( x, y )
         ...
         double precision a, b, x, y, TWOPI
